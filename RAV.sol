...
contract Rav is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 20000 ether;
    // Initial distribution for the day 2-5 RAV-WADA LP -> RAV pool
    uint256 public constant INITIAL_RAV_POOL_DISTRIBUTION = 20000 ether;
    ...
    /**
     * @notice Constructs the RAV ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("RAV", "RAV") {
        // Mints 1 RAV to contract creator for initial pool setup
    ...
    function _getRavPrice() internal view returns (uint256 _tombPrice) {
        try IOracle(ravOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Rav: failed to fetch RAV price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _ravPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_ravPrice >= taxTiersTwaps[tierId]) {
    ...
    function setRavOracle(address _ravOracle) public onlyOperatorOrTaxOffice {
        require(_ravOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints RAV to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of RAV to mint to
    ...
    function transfer(
        ...
        if (autoCalculateTax) {
            uint256 currentRavPrice = _getRavPrice();
            currentTaxRate = _updateTaxRate(currentRavPrice);
            if (currentRavPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_ravPool != address(0), "!_ravPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_ravPool, INITIAL_RAV_POOL_DISTRIBUTION);
